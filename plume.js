	...
	function _render(sel, obj) {
			...
			var parseIf = function() {
				...
				if (_val) {
					...
					if (func) {
						...
						evalExpr =
							!!(get(ctx, condition[0]) || get(localCtx, condition[0])) == type;
						props.push(condition[0].split(".")[0]);
						propvalues.push(get(ctx, props[0]));
					}
		...
		var bindText = function(pel, el, ctx, localCtx) {
			var node_val = el.nodeValue.replace(/[\r\n]/g, "").trim(),
				bindProp = "",
				targetctx = Object.assign(
					{},
					copy(ctx),
					copy(localCtx !== undefined ? localCtx : {})
				);
			if (node_val !== "") {
				el.nodeValue = node_val.replace(
					/\{\{\s?([\w.]+)\s?\}\}/g,
					(match, variable) => {
		...
		var bindAttributes = function(pel, el, ctx, localCtx) {
			if (el.attributes.length > 0) {
				...
				for (var i = 0; i < _attrs.length; i++) {
					if (
						_attrs[i].specified &&
						["class", "style"].indexOf(_attrs[i].name) < 0
					) {
		...
		this.render = function() {
			...
			if (!obj.controller) {
				throw Error(
					"controller is required and should be a function or an array"
				);
			}
